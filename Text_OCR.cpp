// OpenCV_32_bit.cpp : Defines the entry point for the console application.
//
/*
18-03-2018
Author: Sahil Malik
Technology Used:OpenCV 3.0.0, Tesseract 3.0.2, C++11 
*/
#include "stdafx.h"
#include <time.h>
...
{	
	...
	if(ocr.MeanTextConf()<=45)	return ch;
    // Get the text    
	return text;
}
...
int main(int argc, char* argv[]) 
{
	...
	std::ofstream file; 
	file.open("ticketHigh.txt", std::ios::out | std::ios::binary);

	//For each region
	for (auto& region : regions) {
		//Crop 
		auto cropped = deskewAndCrop(ticket, region);
		char* text = identifyText(cropped, "eng");
	...
	int stop_s=clock();
	cout<<"CLOCK RATE: "<<CLOCKS_PER_SEC<<endl;
	cout<<"TIME EXECUTION: "<<(stop_s-start_s)/double(CLOCKS_PER_SEC)*1000<<endl;
